...
namespace Dwares.Rookie.Airtable
{
	public class AirBase
	{
		...
		const int MAX_NUMBER_OF_RECORDS_IN_LIST = 100;

		public AirBase(string apiKey, string baseId)
		{
			//Debug.EnableTracing(@class);
			Predicate.ArgumentNotEmpty(apiKey, nameof(apiKey));
			Predicate.ArgumentNotEmpty(baseId, nameof(baseId));
		...
		{
			Predicate.ArgumentNotNull(table, nameof(table));
		...
		{
			Predicate.ArgumentNotNull(table, nameof(table));
			Predicate.ArgumentIsInRange(maxRecords, 1, MAX_NUMBER_OF_RECORDS_IN_LIST, nameof(maxRecords));
